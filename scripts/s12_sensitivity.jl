## s12_sensitivity.jl — Sensitivity analysis for DIPCM parameters
#
# Tests the DIPCM two-term update: x_{n+1} = α z_n + σ_n u_n
# on Example 1 (VIP, x* = 0) with moderate L (W ~ U(0,1)).
#
# Fixed:  β_n = 1/(n+1)  (Halpern decay)
# Swept:  α (weight on z_n), θ̄ (inertial bound), β_zn (second inertial)
#
# Usage:
...
#
# Output:
#   results/sensitivity/dipcm_sensitivity_N{dim}.csv
...
function main()
    ...
    logpath, tee, logfile = setup_logging("s12_sensitivity";
        logdir=joinpath("results", "sensitivity"))
