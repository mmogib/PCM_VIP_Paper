...
# 7. Single Simulation - One run: generate data → CV on train → retrain on train+val → test eval
...
#   2. Hyperparameter tuning → find best (λ₁, λ₂) via CV on training set only
#   3. Final training → retrain on combined train+val data with best hyperparams
...
function get_IPCMAS1_params_EN(L::Float64; γ=1.1, μ0=0.5, α0=0.30, β0=0.0119, λ0=0.5)
    a_seq(n) = 100 / (n)^(2)
    θ_seq(n) = 0.9
    return (
        γ=γ,
...
"""
function save_simulation_results(summary::Dict, output_dir::String="results/example_3";
    ...
    if clearfolder
        clear_folder_recursive("results/example_3"; clearSubfolders=false)
    end
    if !isdir(output_dir)
        mkpath(output_dir)
    end
...
2. Use CV on training set (n_train samples) to select hyperparameters
3. Retrain final model on combined train+val (n_train + n_val samples) with best hyperparameters
4. Evaluate on test set (n_test samples)

This ensures all available training data is used for the final model while maintaining
proper separation between hyperparameter selection and model evaluation.
"""
function run_single_simulation(algorithm, params_function, n_train, n_val, n_test, n_features;
    ...
    # 2. Select hyperparameters using cross-validation on training data only
    start_time = time()
    ...
    # 3. Retrain on combined train+val data with best parameters
    # This uses all available data for the final model after hyperparameter selection
    X_trainval = vcat(data.X_train, data.X_val)
    y_trainval = vcat(data.y_train, data.y_val)

    start_time = time()
    ŵ = solve_elastic_net(X_trainval, y_trainval,
    ...
    return (
        NamedTuple(map(((i, y),) -> Symbol("w$i") => y, enumerate(ŵ)))...,
        λ₁=best_params.λ₁,
        λ₂=best_params.λ₂,
...
function plotit(; clearfolder::Bool=true, output_dir::String="results/example_3")

    # Read the results
    latest_csv = find_newest_csv(output_dir, "elastic_net_simulation")
    df = CSV.read(latest_csv, DataFrame)
    # Prepare output directory
    if clearfolder
        printstyled("Clearing Plots folder", color=:red, bold=true)
        clear_folder_recursive("$output_dir/plots"; clearSubfolders=false)
    end
